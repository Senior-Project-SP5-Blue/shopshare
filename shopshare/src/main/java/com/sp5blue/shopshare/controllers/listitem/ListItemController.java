...
public class ListItemController implements ListItemControllerBase {
  ...
  public ResponseEntity<ListItemDto> addListItem(
      ...
      @RequestBody @Valid CreateListItemRequest createListItemRequest) {
    return ResponseEntity.ok()
        .body(
            listItemService
                .addListItemToList(userId, groupId, listId, createListItemRequest)
                .join());
  }
  ...
  public ResponseEntity<ListItemDto> getListItem(
      @PathVariable("user_id") UUID userId,
      @PathVariable("group_id") UUID groupId,
      @PathVariable("list_id") UUID listId,
      @PathVariable("list-item_id") UUID itemId) {
    return ResponseEntity.ok()
        .body(listItemService.getListItemById(userId, groupId, listId, itemId).join());
  }

  @Override
  @PatchMapping("/{list-item_id}")
  public ResponseEntity<ListItemDto> changeListItem(
      @PathVariable("user_id") UUID userId,
      @PathVariable("group_id") UUID groupId,
      @PathVariable("list_id") UUID listId,
      @PathVariable("list-item_id") UUID itemId,
      @RequestBody @Valid EditListItemRequest editListItemRequest) {
    var editedItem =
        listItemService.editListItem(userId, groupId, listId, itemId, editListItemRequest).join();
