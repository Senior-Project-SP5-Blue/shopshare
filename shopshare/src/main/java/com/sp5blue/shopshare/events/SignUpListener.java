...
public class SignUpListener implements ApplicationListener<OnSignUpCompleteEvent> {
    ...
    public SignUpListener(JwtService jwtService, ITokenService tokenService, IMailService mailService) {
    ...
    public void sendConfirmSignUp(OnSignUpCompleteEvent event) {
        ...
        try {
            mailService.sendHTMLMessage(recipientAddress, subject, String.format(htmlMessage, confirmationUrl));
