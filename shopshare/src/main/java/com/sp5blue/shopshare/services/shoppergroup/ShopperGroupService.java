...
public class ShopperGroupService implements IShopperGroupService {
  ...
  @Autowired
  public ShopperGroupService(
      ShopperGroupRepository shopperGroupRepository, IUserService userService) {
  ...
  @Transactional
  @Async
  public CompletableFuture<List<SlimShopperGroupDto>> getShopperGroups(UUID userId) {
    var _groups = shopperGroupRepository.findAllByUserId(userId);
    var groups = _groups.stream().map(SlimShopperGroupDto::new).toList();
  ...
  @Transactional
  @Async
  public CompletableFuture<ShopperGroupDto> getShopperGroupById(UUID userId, UUID groupId)
  ...
  @Async
  @Transactional
  public CompletableFuture<ShopperGroup> readShopperGroupById(UUID userId, UUID groupId)
      throws GroupNotFoundException {
    var group =
        shopperGroupRepository
            .findByUserIdAndId(userId, groupId)
  ...
  @Transactional
  @Async
  public void deleteShopperGroup(UUID userId, UUID groupId)
  ...
  @Transactional
  @Async
  public CompletableFuture<ShopperGroup> updateShopperGroupName(
      UUID userId, UUID groupId, String newName) {
    ShopperGroup shopperGroup =
        shopperGroupRepository
            .findById(groupId)
  ...
  @Transactional
  @Async
  public CompletableFuture<Boolean> addUserToShopperGroup(UUID groupId, UUID shopperId)
      throws GroupNotFoundException {
    ShopperGroup group =
        shopperGroupRepository
            .findById(groupId)
  ...
  @Transactional
  @Async
  public CompletableFuture<Boolean> addUserToShopperGroup(UUID groupId, User user)
      throws GroupNotFoundException {
    ShopperGroup group =
        shopperGroupRepository
            .findById(groupId)
  ...
  public CompletableFuture<UserDto> getShopperGroupUser(UUID userId, UUID groupId, UUID memberId) {
    ...
    return CompletableFuture.completedFuture(
        new UserDto(userService.getUserByShopperGroup(groupId, memberId).join()));
  }

  @Override
  @Transactional
  @Async
  public CompletableFuture<Boolean> removeUserFromShopperGroup(
      UUID userId, UUID groupId, UUID shopperId)
    ...
    if (!shopperGroup.getAdmin().getId().equals(userId))
      throw new InvalidUserPermissionsException(
          "User - " + userId + " does not have permission to remove from group");
    return CompletableFuture.completedFuture(shopperGroup.removeUser(shopperId));
  }
  ...
  @Override
  @Async
  public CompletableFuture<Boolean> userExistsInGroup(String username, UUID groupId) {
    return CompletableFuture.completedFuture(
        userService.userExistsByGroup(username, groupId).join());
  }
  ...
  public ShopperGroup verifyUserHasGroup(UUID userId, UUID groupId) {
    return shopperGroupRepository
        .findByUserIdAndId(userId, groupId)
        .orElseThrow(() -> new GroupNotFoundException("Shopper group does not exist - " + groupId));
  }
}
