...
interface CreateListModalProps {
  ...
  const CreateListsScreen = (props: CreateListModalProps) => {
    ...
    const backgroundColors = ["#5CD859", "#24A6D9","#595BD9", "#8021D9", "#D159D8","#D85963","#D88559"]
    const bgColors = backgroundColors[0]
    function renderColors(){
        return backgroundColors.map(color =>{
            return (
                <TouchableOpacity key={color} style={{backgroundColor:color, width: 30, height: 30, borderRadius: 4 }} onPress={() => ({color})}/>
            )
        })
    }
    return (
        <KeyboardAvoidingView behavior="padding" style={{
            ...
            <TouchableOpacity onPress={lists} style={{position: "absolute", top: 64, left: 32}}>
            <Text style={{ 
                fontSize: 18, 
                color: COLORS.black, 
                fontWeight: "bold"}}>
            ...
            <View style={{alignSelf:"stretch", marginHorizontal:32}}>
                ...
                <TextInput placeholder="New List Name" onChangeText={text =>({name: text})} style={{
                    borderWidth: 1,
